...
# 1. 滿足程式碼要求一：st.header("你的命名：視覺化議題")
st.header("📊 學生運動員成長、機會、抱負與學習經驗：臺灣版 (GOALS-TW)調查資料分析")

# 2. 滿足程式碼要求二：st.subheader("114-2 運動大數據與視覺化分析專題研究/姓名OOO")
st.subheader("114-2 運動大數據與視覺化分析專題研究/王暐捷")

# 3. 滿足其他說明資訊 (要求六：使用 st.expander, st.caption, st.write)
with st.expander("ℹ️ 關於本研究與大專運動員數據保護說明"):
    st.write("""
     國科會NSTC 114-2410-H-022-MY2
    **研究背景與計畫說明**：
    本資料庫為國科會專案計畫「學生運動員成長、機會、抱負與學習經驗：臺灣版 (GOALS-TW)」之調查數據成果。
    透過大數據視覺化技術，探索臺灣地區大學運動員在學業投入、運動訓練、生活滿意度及心理健康等八大構面的核心指標與現況。
    
    **隱私與法律合規聲明 (資料保護)**：
    - **去識別化處理**：本系統所讀取之數據皆已進行去識別化處理，已完全剔除具備個人可追溯性之敏感個資。
    - **群體統計呈現**：所有分析頁面（如長條圖、圓餅圖、雷達圖）僅進行去中心化的群體次數分配與平均數計算，不提供任何單一敏感樣本的個案查詢。
    - **學術專用限制**：本儀表板僅供 114-2 學年度「運動大數據與視覺化分析專題研究」課程期末成果發表使用，嚴禁外流或用於商業用途。
    """)
    st.caption("🔒 若你對本研究、問卷或調查程序有任何疑問，本研究已由國立臺東大學委託國立成功大學人類研究倫理審查委員會審查通過，若欲諮詢研究參與權益或提出申訴，請您聯絡該委員會，電話:06-275-7575＃51020，email：em51020@email.ncku.edu.tw")
